...
(() => {
    ...
    async function publishDataset() {
        ...
        setState({
            isPublishing: true,
            message: 'Импортируем данные в unified каталог (items)...'
        });
        ...
        for (let rowIndex = 0; rowIndex < state.rows.length; rowIndex++) {
            ...
            columns.forEach((colName, colIndex) => {
                ...
                if (!target) return;
                record[target] = row[colIndex];
            });

            const payload = {
                sku: record.sku || record.code || null,
                name: record.name || record['Название'] || 'Товар без названия',
                description_short: record.short_description || record.description || null,
                description: record.description || null,
                attributes: record,
                is_visible: record.is_visible !== undefined ? Boolean(record.is_visible) : true,
