...
class RecipesManagementModule {
    ...
    async loadProducts() {
        try {
            // Загружаем товары из инвентарной системы (ингредиенты и полуфабрикаты)
            const response = await fetch('/api/inventory/products');
            if (response.ok) {
                const data = await response.json();
                if (data.ok && Array.isArray(data.data)) {
                    // Фильтруем только ингредиенты и полуфабрикаты
                    this.products = data.data.filter(p => 
                        p.type === 'ingredient' || 
                        p.type === 'semi_product' || 
                        p.type === 'material'
                    );
