...
(function() {
    ...
    const ProductCardsManager = {
        ...
        renderModifiersForEdit(product) {
            // Получаем выбранные модификаторы из товара
            const productModifiers = product.modifiers || product.mods || [];
            const selectedGroupIds = Array.isArray(productModifiers) 
                ? productModifiers.map(m => m.group_id || m.id || m).filter(Boolean)
                : [];
            ...
            return `
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 1rem;">
                    ${this.modifiers.map(group => {
                        const isChecked = selectedGroupIds.includes(group.group_id) || selectedGroupIds.includes(String(group.group_id));
                        return `
                                        ...
                                        ${group.options && group.options.length > 0 ? `
                                            <div style="margin-top: 0.5rem; padding-top: 0.5rem; border-top: 1px solid #e5e7eb;">
                                                ${group.options.slice(0, 3).map(opt => 
                                                    `<div style="font-size: 0.75rem;">• ${this.escapeHtml(opt.name)} ${opt.price > 0 ? `(+${opt.price}₽)` : ''}</div>`
                                                ).join('')}
                                                ${group.options.length > 3 ? `<div style="font-size: 0.75rem; color: #9ca3af;">...и еще ${group.options.length - 3}</div>` : ''}
                                            </div>
                                        ` : ''}
                                    </div>
                                </div>
        ...
        async loadProducts() {
            ...
            try {
                // 1) Пробуем API с максимальным лимитом
                let loaded = [];
                try {
                    ...
                    if (response.ok) {
                        ...
                        // Отладка: проверяем первый товар
                        if (loaded.length > 0) {
                ...
                this.products = (loaded || []).map(p => {
                    ...
                    return {
                        id: p.id || p.sku || p.code || p.name,
                        name: p.name,
                        description: p.description || p.desc || '',
                        price: parseFloat(p.price) || 0,
                        cost: cost,
                        image_url: p.image_url || p.picture || p.photo || p.image || '',
                        ...
                        visible_on_site: p.visible_on_site !== false && p.available !== false,
        ...
        async syncToWebsite(showNotification = false, productsOverride = null) {
            try {
                ...
                // Подготавливаем товары для синхронизации
        ...
        renderProductsTable() {
            ...
            if (this.products.length === 0) {
                html += `
                    <tr>
                        <td colspan="11" style="text-align: center; padding: 3rem; color: rgba(119, 124, 124, 1); border-bottom: 1px solid rgba(94, 82, 64, 0.12);">
                ...
                // Apply pagination logic to filter products
                ...
                visibleProducts.forEach(product => {
                    ...
                    html += `
                        <tr style="padding: 12px 16px; ${isSelected ? 'background: rgba(94, 82, 64, 0.08);' : ''}" class="product-row" data-product-id="${product.id}" ${isSelected ? 'data-selected="true"' : ''}>
                            ...
                            <td style="padding: 12px 16px; text-align: left; border-bottom: 1px solid rgba(94, 82, 64, 0.12);">${product.price ? product.price.toLocaleString('ru-RU') + ' ₽' : '—'}</td>
                ...
                <div style="background: white; border-radius: 16px; padding: 2rem; max-width: 900px; width: 95%; max-height: 90vh; overflow-y: auto;">
                    ...
                    <div style="margin-top: 2rem; padding-top: 2rem; border-top: 2px solid #e5e7eb;">
                        ...
                        <div id="modifiersContainer" style="margin-bottom: 1rem; padding: 1rem; background: #f9fafb; border-radius: 8px; border: 2px solid #e5e7eb;">
                            ${this.renderModifiersForEdit(product)}
                        </div>
        ...
        async saveEditedProduct(productId) {
            ...
            const productType = document.getElementById('editProductType')?.value || 'product';
            ...
            const calories = document.getElementById('editProductCalories').value.trim();
            ...
            const selectedCategoryIds = Array.from(document.querySelectorAll('.edit-category-checkbox:checked'))
                .map(cb => parseInt(cb.value));
            
            // Собираем категорию номенклатуры
            const categoryStockId = document.getElementById('editProductCategoryStock')?.value || null;
            
            // Собираем группу товара
            const groupId = document.getElementById('editProductGroupId')?.value || null;
            ...
            })).filter(v => v.name); // Убираем пустые варианты
            ...
            const lossPercentEl = document.getElementById('editProductLossPercent');
            const lossPercent = lossPercentEl ? (parseFloat(lossPercentEl.value) || 0) : 0;
            const conversionFactorEl = document.getElementById('editProductConversionFactor');
            ...
            const productData = {
                ...
                type: productType,
                sku: sku,
                short_description: document.getElementById('editProductShortDesc').value.trim(),
                full_description: document.getElementById('editProductFullDesc').value.trim(),
                ...
                weight: document.getElementById('editProductWeight').value.trim(),
                calories: document.getElementById('editProductCalories').value.trim(),
                ingredients: document.getElementById('editProductIngredients').value.trim(),
                allergens: document.getElementById('editProductAllergens').value.trim(),
                ...
                recommended_products: JSON.stringify(recommended),
                // ✅ КРИТИЧНО: Сохраняем выбранные модификаторы
                modifiers: (() => {
                    const checkedModifiers = Array.from(document.querySelectorAll('.edit-modifier-checkbox:checked'))
                        .map(cb => {
                        ...
                        .filter(Boolean);
                    return JSON.stringify(checkedModifiers);
                })(),
                loss_percentage: lossPercent,
                conversion_factor: conversionFactor,
                vat_rate: document.getElementById('editProductVatRate').value || null,
                tax_type: document.getElementById('editProductTaxType').value || 'none'
            };
        ...
        async handleFileSelect(file) {
            ...
            try {
                ...
                const fileType = file.name.toLowerCase().endsWith('.csv') ? 'csv' : 
                               file.name.toLowerCase().endsWith('.yml') || file.name.toLowerCase().endsWith('.yaml') ? 'yml' : 'xml';
                
                // Валидация количества строк (до 10 000 согласно ТЗ)
                const lineCount = text.split('\n').filter(line => line.trim()).length;
                if (lineCount > 10000) {
                    alert(`⚠️ Файл содержит ${lineCount} строк. Будут обработаны первые 10 000 строк.`);
                }
                ...
                if (fileType === 'csv') {
                    ...
                    alert('❌ Неподдерживаемый формат файла. Используйте CSV или YML');
                    if (importBtn) importBtn.disabled = true;
                    return;
                }
        ...
        async startImport() {
            ...
            try {
                ...
                // Собираем сопоставление полей, если оно было настроено
                const fieldMapping = {};
                if (this.importData.type === 'csv' && this.importData.headers) {
                    // Автоматическое сопоставление на основе заголовков
                    this.importData.headers.forEach(header => {
                        const systemField = this.autoMapFieldToSystem(header);
                        if (systemField) {
                            fieldMapping[systemField] = header;
                        }
                    });
                }
                ...
                const message = result.message || `Импорт завершен. Создано: ${result.created || 0}, Обновлено: ${result.updated || 0}, Ошибок: ${result.errors || 0}`;
                alert(`✅ ${message}`);
                ...
                switch(type) {
                    ...
                    case 'modifiers':
                        ...
                        try {
                            ...
                            if (response.ok) {
                                const result = await response.json();
                                data = (result.data || []).map(mod => ({
                                    category_menu: mod.category_menu || '',
                                    name: mod.name || '',
                                    price: mod.price || 0,
                                    applied_to: mod.applied_to || '',
                                    type: mod.type || 'modifier'
                                }));
                            }
                        } catch (e) {
                            console.warn('API модификаторов не доступен');
                        }
                ...
                <table style="width: 100%; border-collapse: collapse;" id="categoriesTable">
                    <thead>
                        <tr style="background: #f8f9fa; border-bottom: 2px solid #dee2e6;">
                            ...
                            <th style="padding: 12px; text-align: left; font-weight: 600;">Slug</th>
