...
class AggregatorIntegrationsModule {
        ...
        root.innerHTML = `
            <div class="card">
                <div class="card-header">
                    ...
                    <p class="card-subtitle">Управление интеграциями с Яндекс.Еда, Delivery Club и ВкусВилл (демо режим без API ключей)</p>
                </div>
        ...
        try {
            const response = await fetch(`${this.API_BASE}/${id}/sync`, {
                ...
                body: JSON.stringify({
                    ...
                    revenue_today: Math.floor(Math.random() * 15000),
                    status: 'active'
                })
            });
            const result = await response.json();
            ...
            alert('✅ Обновление меню выполнено (демо режим)');
            await this.loadData();
            this.render();
        } catch (error) {
    ...
    async saveAggregator(id) {
        ...
        try {
            ...
            alert('✅ Настройки агрегатора сохранены (демо режим)');
            modal?.remove();
            await this.loadData();
            this.render();
        } catch (error) {
